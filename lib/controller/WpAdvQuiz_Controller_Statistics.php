...
class WpAdvQuiz_Controller_Statistics extends WpAdvQuiz_Controller_Controller
{
    ...
    private function showNew($quizId)
    {
        ...
        $view = new WpAdvQuiz_View_StatisticsNew();
    ...
    public function save($quiz = null)
    {
        ...
        if ($quiz->getStatisticsIpLock() > 0) {
            ...
            $lockMapper->deleteOldLock($lockTime, $quiz->getId(), time(), WpAdvQuiz_Model_Lock::TYPE_STATISTIC);

            if ($lockMapper->isLock($quizId, $lockIp, $userId, WpAdvQuiz_Model_Lock::TYPE_STATISTIC)) {
                return false;
            }
            ...
            $lock->setQuizId($quizId)
                ->setLockIp($lockIp)
                ->setUserId($userId)
                ->setLockType(WpAdvQuiz_Model_Lock::TYPE_STATISTIC)
                ->setLockDate(time());

            $lockMapper->insert($lock);
        }

        $statisticRefModel = new WpAdvQuiz_Model_StatisticRefModel();
    ...
    private function makeFormData($quiz, $data)
    {
        ...
        foreach ($forms as $form) {
            if ($form->getType() != WpAdvQuiz_Model_Form::FORM_TYPE_DATE) {
                ...
                if (!WpAdvQuiz_Helper_Form::valid($form, $str)) {
                    return null;
                }
                ...
                $dateStr = WpAdvQuiz_Helper_Form::validData($form, $date);
    ...
    private function makeDataList($quizId, $array, $modus)
    {
        ...
        if ($modus == WpAdvQuiz_Model_Quiz::QUIZ_MODUS_SINGLE) {
        ...
        foreach ($array as $k => $v) {
            $s = new WpAdvQuiz_Model_Statistic();
            $s->setQuestionId($k);
    ...
    public static function ajaxLoadHistory($data)
    {
        ...
        foreach ($statisticModel as $model) {
            ...
            if (!$model->getUserId()) {
                ...
                if ($model->getUserName() == '') {
                    $model->setUserName(__('Deleted user', 'wp-adv-quiz'));
                }
            }
            ...
            $model->setFormatTime(WpAdvQuiz_Helper_Until::convertTime($model->getCreateTime(),
                get_option('wpAdvQuiz_statisticTimeFormat', 'Y/m/d g:i A')));
            ...
            foreach ($forms as $form) {
                ...
                if ($form->isShowInStatistic()) {
                    $formOverview[] = $formData != null && isset($formData[$form->getFormId()])
                        ? WpAdvQuiz_Helper_Form::formToString($form, $formData[$form->getFormId()])
                        : '----';
                }
            }
        ...
        $view = new WpAdvQuiz_View_StatisticsAjax();
    ...
    public static function ajaxLoadStatisticUser($data)
    {
        ...
        foreach ($statisticUsers as $statistic) {
            /* @var $statistic WpAdvQuiz_Model_StatisticUser */
        ...
        $view->userName = __('Anonymous', 'wp-adv-quiz');
        ...
        if ($view->statisticModel->getUserId()) {
            ...
            if ($userInfo !== false) {
                ...
                $view->userName = __('Deleted user', 'wp-adv-quiz');
            }
        }
    ...
    {
        if (!current_user_can('wpAdvQuiz_reset_statistics')) {
            return;
        }
